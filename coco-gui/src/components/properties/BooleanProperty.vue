<template>
  <n-checkbox v-model:checked="value" :disabled="props.disabled"
    @update:checked="(v: boolean | null) => emit('update', v)"> {{ props.par.name }} </n-checkbox>
</template>
...
const emit = defineEmits<{ (event: 'update', value: boolean | null): void; }>();
