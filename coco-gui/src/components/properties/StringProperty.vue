<template>
  <n-input v-model:value="value" :placeholder="props.par.name" :disabled="props.disabled" clearable
    @update:value="(v: string | null) => emit('update', v)" />
</template>

<script setup lang="ts">
import { NInput } from 'naive-ui';
import { taxonomy } from '@/taxonomy';
import { ref, watch } from 'vue';

const props = withDefaults(defineProps<{ par: taxonomy.StringProperty; value: string | undefined; disabled: boolean; }>(), { disabled: false });
const emit = defineEmits<{ (event: 'update', value: string | null): void; }>();

const value = ref<string | undefined>(props.value);

watch(() => props.value, (new_value: string | undefined) => value.value = new_value);
</script>