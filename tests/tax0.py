...
def init_db(url):
    ...
    # Create some users
    response = requests.post(url + '/items', json={'type': user_type['id'], 'name': 'alice@ageit.it', 'properties': {'name': 'Alice', 'gender': 'F'}})
    alice = response.json()
    print(alice)
    response = requests.post(url + '/items', json={'type': user_type['id'], 'name': 'bob@ageit.it', 'properties': {'name': 'Bob', 'gender': 'M'}})
    bob = response.json()
    print(bob)

    # Create some kits
    response = requests.post(url + '/items', json={'type': kit_type['id'], 'name': 'kit1'})
    kit1 = response.json()
    print(kit1)
    response = requests.post(url + '/items', json={'type': kit_type['id'], 'name': 'kit2'})
    kit2 = response.json()
    print(kit2)

    # Create some sensors
    response = requests.post(url + '/items', json={'type': rppg_sensor_type['id'], 'name': 'rppg1', 'properties': {'kit': kit1['id']}})
    rppg1 = response.json()
    print(rppg1)
    response = requests.post(url + '/items', json={'type': wws_sensor_type['id'], 'name': 'wws1', 'properties': {'kit': kit1['id']}})
    wws1 = response.json()
    print(wws1)
    response = requests.post(url + '/items', json={'type': swf_sensor_type['id'], 'name': 'swf1', 'properties': {'kit': kit2['id']}})
    swf1 = response.json()
    print(swf1)
    response = requests.post(url + '/items', json={'type': garmin_sensor_type['id'], 'name': 'garmin1', 'properties': {'kit': kit2['id']}})
